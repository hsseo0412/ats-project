...
export default function Edit({ jobPosting }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        patch(route('job-postings.update', jobPosting.id));
    };

    return (
        <AuthenticatedLayout header={
            <h1 className="text-base font-semibold text-slate-900">채용공고 수정</h1>
        }>
            <Head title="채용공고 수정" />
                    ...
                    <div className="rounded-xl bg-white p-6 ring-1 ring-slate-200">
                        <form onSubmit={submit} className="space-y-5">
                            ...
                            <div>
                                ...
                                <textarea id="description" rows={5} className={inputClass} value={data.description} onChange={(e) => setData('description', e.target.value)} required />
                                <InputError message={errors.description} className="mt-1" />
                            </div>

                            <div>
                                <label htmlFor="requirements" className="block text-sm font-medium text-slate-700 mb-1.5">자격 요건</label>
                                <textarea id="requirements" rows={4} className={inputClass} value={data.requirements} onChange={(e) => setData('requirements', e.target.value)} required />
